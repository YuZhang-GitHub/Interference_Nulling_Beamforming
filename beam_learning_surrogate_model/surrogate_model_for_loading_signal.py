...
class GainPred_signal(nn.Module):

    def __init__(self, M):
        super(GainPred_signal, self).__init__()

        self.M = M
        self.pi = torch.tensor(np.pi).float().cuda()

        self.H_r = nn.Parameter(torch.randn(self.M, 2))
        self.H_i = nn.Parameter(torch.randn(self.M, 2))
