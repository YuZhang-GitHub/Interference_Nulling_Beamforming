...
projection = ctranspose(F)*vec; % the column of f.' is the desired beam
...
%     plot(theta_s*(180/pi), proj(:,n).');
    polarplot(theta_s, proj(:,n).')
%     plot(theta_s*(180/pi), 10*log10(proj(:,n).'))
    grid on
    box on
%     xlim([0, 180])
%     polarplot(theta_s, proj(:,n).', 'k');
    hold on;
end
